...
app.post('/api/chat', async function (req, res) {
  ...
  var systemPrompt = [
    ...
    '• Dashboard: visão geral com saldo, resumo do mês, últimas transações, lembretes, widget de metas, widget de investimentos e saldo previsto com recorrentes futuras.',
    ...
    '• Carteira (Contas): gerenciar contas bancárias, poupança, benefícios (vale alimentação, etc). Importar extratos CSV/OFX/PDF/imagem. Ver histórico de importações anteriores (data, tipo, quantidade de transações).',
    '• Transferências: mover saldo entre contas cadastradas. Gera dois lançamentos automáticos (saída na conta de origem, entrada na conta destino). Botão "Transferir" na tela Carteira.',
    '• Cartões: gerenciar múltiplos cartões de crédito. Acompanhar fatura atual, limite disponível, parcelas em aberto e parcelamento inteligente via IA. Pagar fatura diretamente de uma conta cadastrada.',
    '• Recorrentes: lançar automaticamente contas fixas (aluguel, streaming, etc.) todo mês no dia configurado. Suporta parcelas restantes (ex: financiamento em X vezes).',
    '• Dívidas: controlar empréstimos e financiamentos com Tabela Price, simulador de quitação antecipada e acompanhamento de parcelas pagas/restantes.',
    ...
    '• Insights: score de saúde financeira 0-100, alertas automáticos, projeções e dicas personalizadas.',
    ...
    '• Mercado: lista de compras inteligente com estimativa de valor.',
    ...
    '• Configurações: mudar plano, tema de cor (8 opções: Padrão Gelo, Dark HBO e 6 temas coloridos), foto de perfil, ocultar valores (privacidade).',
    '',
