...
| PEND-049 | **Assistente IA — Function calling / Tool use** — permitir que a IA execute ações no app (ex: "criar meta de R$5000 em 6 meses", "marcar dívida X como paga"). Hoje a IA só responde texto. | 🟡 Média | Solicitado em 29/04/2026 (Assistente IA v2) | Groq llama-4-scout suporta tool use. Requer definir schema de tools no backend + handler no frontend. |
| PEND-050 | **Assistente IA — Histórico persistente** — salvar conversas em `usuarios/{uid}/chats/{chatId}` no Firestore (hoje só fica em sessionStorage e some ao fechar aba). Permitir buscar conversas antigas. | 🟢 Baixa | Solicitado em 29/04/2026 (Assistente IA v2) | Cuidado com custo de armazenamento — limitar a últimas N conversas. |
| PEND-051 | **Assistente IA — Reconhecimento de voz** — botão de microfone para ditar mensagem (Web Speech API). | 🟢 Baixa | Solicitado em 29/04/2026 (Assistente IA v2) | Web Speech API é suportada em Chrome/Edge — fallback para mobile. |
