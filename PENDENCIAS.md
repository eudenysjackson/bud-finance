...
| PEND-046 | **FCM Push Notifications — Sistema Completo** — design aprovado em 21/05/2026. **Camada 1 (OS push)**: Service Worker + Web Push API + FCM. Permissão pedida via **popup próprio Bud** no primeiro login (não direto o popup do OS). Token salvo em `usuarios/{uid}/fcmToken`. Cron diário no backend (9h) envia notificações variadas: (a) lembrete de registro casual ("🛵 Andou de Uber? Registra aqui"); (b) recorrente vencendo em 1 dia; (c) limite de categoria em ≥80%; (d) revisão semanal de gastos; (e) progresso de meta; (f) plano expirando em 2–3 dias; (g) trial acabando. Ícone: ícone do app + branding Buddy. **Camada 2 (in-app)**: ver PEND-084. **Implementação**: (1) criar `appbudfinance/sw.js` (Service Worker); (2) registrar SW em `bud-utils.js`; (3) endpoint `POST /api/push/token` no backend; (4) endpoint `GET /api/push/enviar-diario` (cron); (5) configurar FCM no Firebase Console; (6) ativar botão em `configuracoes.html` (hoje exibe "Em breve"). | 🔴 Alta | Firebase FCM, Service Worker, cron backend | Design aprovado — pronto para implementar. Bloqueia PEND-032, PEND-036, PEND-041, PEND-047, PEND-066. |
...
| PEND-084 | **In-App Notification Banners (Dashboard)** — design aprovado em 21/05/2026. Banner sutil de **exibição única** (localStorage), posicionado na header do Dashboard **ao lado do botão do olho** (ocultar valores). NÃO bloqueia conteúdo. Fecha com ×. **Prioridade 1 (primeira versão)**: (a) Boas-vindas: `bud_notif_welcome` → "👋 Bem-vindo(a) ao Bud Finance, [Nome]! Que tal fazer o tutorial?" — dispara no primeiro login; (b) Plano expirando amanhã: `bud_notif_expire_${uid}` → "🔔 Seu plano expira amanhã — que tal fazer upgrade?" — verificar no `carregarDadosUsuario()`. **Fase 2**: trial acabando em 3 dias; sem lançamentos há 7 dias; suporte disponível. **Implementação**: criar `js/bud-notif-inapp.js` com funções `initInAppNotif(user, userData)` + `dismissNotif(key)`. Injetar banner via `style.cssText` (NUNCA Tailwind dinâmico). Chamar de `dashboard.js` após carregar perfil. | 🔴 Alta (prioridade 1) | Depende de `dashboard.js` + dados do usuário | Design aprovado. Implementar antes de PEND-046 (não requer FCM). |
