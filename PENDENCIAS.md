...
| PEND-065 | **Histórico de importações no extrato/carteira** — não há tela ou seção exibindo quais arquivos já foram importados (data, nome do arquivo, qtd de transações, conta destino). O `carteira.js` só guarda o hash do arquivo para deduplicação (campo `arquivosImportados` no doc da conta). Proposta: criar sub-coleção `importacoes` no Firestore com campos `nomeArquivo`, `dataImportacao`, `qtdTransacoes`, `contaId` e exibir lista na tela de Contas ou aba no Extrato. | 🟡 Média | Carteira | Solicitado 11/05/2026 |
...
| PEND-053 | **Assistente WhatsApp — Ativar em produção** — Fase 1 (token de pareamento) está implementada no código (`backend/server.js`, `configuracoes.html`, `js/configuracoes.js`). Bloqueado por falta de número WhatsApp dedicado. Passos necessários: (1) comprar chip pré-pago (~R$10) para ser o número do Bud Finance; (2) criar VPS Ubuntu (~R$30/mês Contabo/Hetzner) e instalar Evolution API via Docker; (3) escanear QR Code para vincular o chip; (4) adicionar ENV vars no Render: `WA_EVOLUTION_URL`, `WA_EVOLUTION_KEY`, `WA_NUMERO_DISPLAY`, `WA_NUMERO_LINK`, `WA_VERIFY_TOKEN`; (5) configurar webhook Evolution apontando para `/webhook/whatsapp`. Guia completo disponível na conversa. | 🔴 Alta | Bloqueado por chip/VPS — registrado em 30/04/2026 |
